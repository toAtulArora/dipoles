/*==============================================================================
== FILENAME :
==    USB driver (obdev based) for PC end
================================================================================
==
== FILE NAME :
==    hiddata.c
==
== Description :
==    OBDEV based V-USB driver. Functions for the PC end are here. These are 
==		based upon libusb. As these functions are not called directly by user
==    application program, we do not describe/comment them here in this file.
==    For application programs API see functions in usbIO.c
==
==	Note : 
==		Changed obdev's static char usbErrorMessage() to non-static as it is now
==		exported to usbIO.c. This function has been copied from obdev's hidtool.c
==		file and included here. File hidtool.c is not used.
==
==		Also, struct stUSBdevice is now populated while opening the device. See
==		code added in usbhidOpenDevice().
==
== DATE         NAME              REASON
== ----         ----              ------
== 01Aug2010   Ravi Mehrotra     Original
    ...
    for(i=0;;i++){
        ...
        if(vendorName != NULL && productName != NULL){
            ...
            if(strcmp(vendorName, buffer) != 0)
                continue;
            strcpy( stUSBdevice.acVendor, buffer );	// code added Ravi Mehrotra
            ...
            strcpy( stUSBdevice.acProduct, buffer );	// code added Ravi Mehrotra
            if(strcmp(productName, buffer) != 0)
                continue;
			   HidD_GetSerialNumberString(handle, buffer, sizeof(buffer));
            convertUniToAscii(buffer);
            buffer[ strlen(buffer)/2 ] = 0; // only half the string is OK !!!
            strcpy( stUSBdevice.acSerialNum, buffer );
            stUSBdevice.sVersion = deviceAttributes.VersionNumber;
        }
                    ...
                    if(strcmp(string, vendorName) == 0){
                      ...
                      //      fprintf(stderr, "seen product ->%s<-\n", string);
                           if(strcmp(string, productName) == 0)
                            ...
                            // -------------code added by Ravi Mehrotra
                           {
                            	strcpy( stUSBdevice.acVendor, vendorName );
                            	strcpy( stUSBdevice.acProduct, productName );
										len = usbhidGetStringAscii(handle, dev->descriptor.iSerialNumber, stUSBdevice.acSerialNum, sizeof(stUSBdevice.acSerialNum));
										stUSBdevice.acSerialNum[ len/2 ] = 0; // only half of the string is OK !!!
										stUSBdevice.sVersion = dev->descriptor.bcdDevice;
									// -------------- end code added by Ravi Mehrotra
                              break;
									}
                        }
                    }
